	...
	async run (interaction, client) {
		...
		if(i.isButton()) {
			...
			if(customId === "delete") {
				...
				await i.reply({content: "Le ticket va être supprimé."})
				setTimeout(() => {
		...
		if(i.isStringSelectMenu()) {
			...
			if(i.customId === "ticketsPanel") {
				...
				if(hasTicket) return i.reply({content: `Vous possédez déjà un ticket, ${userTicketChannel}`, ephemeral: true})
				...
				let newChannel = await i.guild.channels.create({
					...
					parent: i.guild.channels.cache.get(config.ticketCategory) || null,
				...
				i.reply({content: `Votre ticket a été crée, ${newChannel}`, ephemeral: true})
				...
				newChannel.send({
					...
					embeds: [
						new Discord.EmbedBuilder()
						.setColor(config.color.bot)
						.setDescription(`Merci d'avoir ouvert un ticket, le staff va bientôt arriver. \n Pour fermer ce ticket, cliquez sur **🔒 Fermer**.`)
					],
					components: [
						new Discord.ActionRowBuilder()
						.addComponents(
